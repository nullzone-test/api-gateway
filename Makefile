...
check-schema:
	...
	@curl -sfL "$$(cat .cache/docs-url)" > /dev/null 2>&1 || (echo "ESCHEMA_MISMATCH: schema registry unreachable (see .cache/docs-url)" && echo "Verify network connectivity to the registry endpoint" && exit 1)
